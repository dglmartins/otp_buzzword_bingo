...
Messages such as `ChannelJoined`, `ChannelClosed`, `ChannelErrored`, and `Heartbeat` are tagged with `DelegatedPhxMsg` and ultimately delegated 
to the `Phoenix.Socket.update` function.

Messages such as `ReceiveGameSummary`, `ReceiveChatMessage`,
`ReceivePresenceState`, `ReceivePresenceDiff`, and `ReceiveError` 
are NOT tagged. These messages are fed into the `update` function, 
just like the messages produced by a button click.
-}
